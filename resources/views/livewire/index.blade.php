...
<div class="flex h-full w-5/6 mx-auto flex-1 flex-col gap-4 rounded-xl">
    <div class="flex flex-col lg:flex-row w-full -mb-2 xl:mb-1 -mt-2 gap-2 items-center">
        ...
        <div class="flex w-full items-center gap-2">
            ...
            <x-mary-button :label="($sortDir == 'asc' ? 'Terdekat' : 'Terjauh')" icon="o-arrows-up-down" wire:click="toggleSort" class="btn-ghost rounded-lg sm:ml-auto" />
        </div>
    </div>
    <flux:separator />
    ...
    <div class="w-full grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-2">
        ...
        @foreach ($jadwals as $jadwal)
            <a href="{{ route('jadwals.show', ['jadwal' => $jadwal->id]) }}" class="cursor-pointer flex flex-col w-full h-full"
                wire:navigate>
                {{-- Film Card --}}
                <x-mary-card
                    ...
                    <div class="flex flex-col h-full w-full justify-end transition-opacity duration-500 ease-in-out opacity-0 hover:opacity-100">
                        <div class="p-4 flex flex-col gap-1/2 text-white/90 font-medium leading-tight h-1/3 bg-linear-to-t from-neutral-950/60 to-neutral-950/80">
                            {{-- Judul Film --}}
                            <span>
                                {{ $jadwal->film->judul }}
                            </span>
                ...
                <div class="text-center mt-2">
                    ...
                    <p class="font-semibold text-lg text-yellow-400">{{ \Carbon\Carbon::parse($jadwal->jam)->format('H:i') }}</p>
                </div>
            </a>
        @endforeach
    </div>
