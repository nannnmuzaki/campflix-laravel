<?php

use App\Models\Film;
use App\Models\Studio;
use App\Models\JadwalTayang;
use Livewire\Volt\Component;
use Livewire\Attributes\{Layout, Title, Rule};
use Illuminate\Validation\Rule as ValidationRule; // Menggunakan alias agar tidak bentrok dengan #[Rule]
use Mary\Traits\Toast;

new
    #[Layout('components.layouts.admin')]
    #[Title('Tambah Jadwal Baru')]
    class extends Component {
    use Toast;

    // --- Properti untuk Model JadwalTayang ---
    #[Rule('required|exists:films,id')]
    public string $film_id = '';

    #[Rule('required|exists:studios,id')]
    public string $studio_id = '';

    #[Rule('required|date')]
    public string $tanggal = '';

    #[Rule('required')]
    public string $jam = '';

    /**
     * Method untuk menyimpan jadwal baru ke database.
     */
    public function store()
    {
        // Validasi semua input form, termasuk validasi unik gabungan
        $validated = $this->validate(
            [
                'film_id' => 'required|exists:films,id',
                'studio_id' => 'required|exists:studios,id',
                'tanggal' => 'required|date',
                'jam' => [
                    'required',
                    // Aturan ini memastikan kombinasi studio_id, tanggal, dan jam adalah unik
                    ValidationRule::unique('jadwal_tayang')->where(function ($query) {
                        return $query->where('studio_id', $this->studio_id)
                            ->where('tanggal', $this->tanggal)
                            ->where('jam', $this->jam);
                    }),
                ],
            ],
            [
                // Pesan error kustom untuk validasi unik
                'jam.unique' => 'Jadwal untuk studio, tanggal, dan jam yang sama sudah ada.'
            ]
        );

        // Buat dan simpan data jadwal baru
        JadwalTayang::create($validated);

        // Tampilkan notifikasi sukses
        $this->toast(
            type: 'success',
            title: 'Jadwal Ditambahkan!',
            description: 'Jadwal tayang baru berhasil disimpan.',
            position: 'toast-bottom',
            icon: 'o-check-circle',
            timeout: 3000
        );

        // Arahkan ke halaman daftar jadwal setelah berhasil
        return $this->redirect(route('admin.jadwal.index'), navigate: true);
    }

    /**
     * Menyiapkan data untuk dikirim ke view.
     */
    public function with(): array
    {
        // Mengambil data film untuk dropdown select
        $films = Film::orderBy('judul')->get()->map(function ($film) {
            return ['id' => $film->id, 'name' => $film->judul];
        })->all();

        // Mengambil data studio untuk dropdown select
        $studios = Studio::orderBy('nama_studio')->get()->map(function ($studio) {
            return ['id' => $studio->id, 'name' => $studio->nama_studio];
        })->all();

        return [
            'films' => $films,
            'studios' => $studios
        ];
    }
}; ?>

<div>
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        {{-- Header halaman --}}
        <x-mary-header title="Tambah Jadwal Baru" subtitle="Pilih film, studio, dan waktu tayang"
            class="dark:text-white/90 mb-2!" separator />

        {{-- Form untuk menambah jadwal baru --}}
        <x-mary-form wire:submit="store">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Pilihan Film --}}
                <div class="md:col-span-full">
                    <x-mary-select label="Film" placeholder="Pilih Film" wire:model="film_id" :options="$films"
                        class="dark:text-white/90 dark:bg-neutral-950 rounded-md" />
                </div>

                <div class="md:col-span-full">
                    {{-- Pilihan Studio --}}
                    <x-mary-select label="Studio" placeholder="Pilih Studio" wire:model="studio_id" :options="$studios"
                        class="dark:text-white/90 dark:bg-neutral-950 rounded-md" />
                </div>

                {{-- Pilihan Tanggal --}}
                <x-mary-datetime label="Tanggal Tayang" wire:model="tanggal"
                    class="dark:text-white/90 dark:bg-neutral-950 rounded-md" icon="o-calendar-days" />

                {{-- Pilihan Jam --}}
                <x-mary-datetime label="Jam Tayang" wire:model="jam" icon="o-clock" type="time"
                    class="dark:text-white/90 dark:bg-neutral-950 rounded-md" />

            </div>

            {{-- Tombol Aksi Form --}}
            <x-slot:actions>
                <x-mary-button label="Batal" link="{{ route('admin.jadwal.index') }}"
                    class="btn-primary dark:bg-neutral-700 rounded-lg border-none hover:bg-neutral-600" wire:navigate />
                <x-mary-button label="Simpan" icon="o-paper-airplane" spinner="store" type="submit"
                    class="btn-primary dark:bg-stone-50 rounded-lg border-none hover:bg-stone-200 text-zinc-800" />
            </x-slot:actions>
        </x-mary-form>
    </div>
</div>